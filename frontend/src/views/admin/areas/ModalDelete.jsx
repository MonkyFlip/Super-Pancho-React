...
// 💡 Asegúrate de importar la función correcta para eliminar áreas
...
const ModalDeleteArea = ({ visible, onClose, onConfirmSuccess, area, tema }) => {
  ...
  const confirm = async () => {
    ...
    setLoading(true);
    try {
      // Manejar el _id como objeto {$oid: "..."} o string
      ...
      const serverMsg = err?.response?.data?.error || err.message || 'Error al eliminar el área';
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ color: '#666', marginTop: 6 }}>
              {/* 💡 Textos actualizados */}
              {`¿Eliminar el área "${area.nombre}" (ID ${typeof area._id === 'object' ? area._id.$oid : area._id})? Esta acción no se puede deshacer.`}
            </div>
...
const dangerBtnStyle = (tema) => ({
  padding: '8px 12px',
  borderRadius: 8,
  background: tema.acento, // Asumiendo que el color de peligro está en 'acento'
  color: '#fff',
  border: 'none',
  cursor: 'pointer',
  fontWeight: 800
