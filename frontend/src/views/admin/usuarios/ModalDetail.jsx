...
const ModalDetail = ({ visible, onClose, usuario, tema }) => {
  ...
  // Normalizaciones locales para mostrar campos coherentes con UsuarioOut
      ...
      <div style={modalStyle(tema)}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <div>
              <div style={{ fontWeight: 900, fontSize: 16 }}>{usuario.usuario ?? usuario.name ?? usuario.usuario_key ?? 'Usuario'}</div>
        ...
        <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div>
            <div style={{ fontSize: 12, color: '#888' }}>Usuario</div>
          ...
          <div>
            <div style={{ fontSize: 12, color: '#888' }}>Rol</div>
            <div style={{ fontWeight: 800 }}>{rol || '-'}</div>
          </div>

          <div>
            <div style={{ fontSize: 12, color: '#888' }}>Estado</div>
            <div style={{ fontWeight: 800 }}>{activo ? 'Activo' : 'Inactivo'}</div>
          </div>

          <div>
            <div style={{ fontSize: 12, color: '#888' }}>Último login</div>
